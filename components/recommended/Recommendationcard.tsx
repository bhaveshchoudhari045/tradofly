...
import { TrendingUp, TrendingDown, Zap, Clock, Target, Shield, ChevronRight, Flame } from 'lucide-react';
...
export default function RecommendationCard({ stock, index, onClick }: RecommendationCardProps) {
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        {ageConf.pulse && <span style={{ width: 5, height: 5, borderRadius: '50%', background: ageConf.color, display: 'inline-block', animation: 'pulseDot 1.5s infinite', boxShadow: `0 0 4px ${ageConf.color}` }} />}
        {ageConf.label}
      </div>
      ...
      <div style={{ paddingTop: 4, paddingLeft: 28 }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 2 }}>
          ...
          }}>{stock.symbol.replace('.NS', '').replace('.BO', '')}</span>
          <span style={{ fontSize: 10, color: 'var(--text-muted)', fontWeight: 500 }}>NSE</span>
        </div>
        <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 12, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 160 }}>
          {stock.name}
        </div>
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
        ...
        <div style={{
          ...
          background: stock.changePercent >= 0 ? `rgba(${palette.buyRgb}, 0.1)` : `rgba(${palette.sellRgb}, 0.1)`,
          color: stock.changePercent >= 0 ? palette.buyColor : palette.sellColor,
      ...
      <div style={{
        ...
        <span style={{ fontSize: 10, color: 'var(--text-muted)', marginLeft: 'auto' }}>
          {stock.daysSinceSignal === 0 ? 'Today' : `${stock.daysSinceSignal}d ago`}
        </span>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 6, marginBottom: 12 }}>
        {[
          { icon: Target, label: 'Target', value: `₹${sig.targetMid.toFixed(1)}`, color: accentColor },
          { icon: Flame, label: 'Gain', value: `+${stock.potentialGain.toFixed(1)}%`, color: accentColor },
          { icon: Shield, label: 'Stop', value: `₹${sig.stopLoss.toFixed(1)}`, color: palette.sellColor },
          ...
          <div key={label} style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 3, marginBottom: 3 }}>
              ...
              <span style={{ fontSize: 9, color: 'var(--text-muted)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{label}</span>
            </div>
      ...
      <div style={{ marginBottom: 10 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
          ...
          <span style={{ fontSize: 10, fontWeight: 800, color: accentColor, fontFamily: 'JetBrains Mono' }}>{confPct}%</span>
        </div>
        <div style={{ height: 3, background: 'var(--border-primary)', borderRadius: 2, overflow: 'hidden' }}>
          <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ fontSize: 10, color: 'var(--text-muted)' }}>
          R:R  <span style={{ fontFamily: 'JetBrains Mono', fontWeight: 700, color: sig.riskRewardRatio >= 2 ? accentColor : 'var(--text-secondary)' }}>
